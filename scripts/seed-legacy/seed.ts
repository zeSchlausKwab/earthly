...
const USER_PROFILES = {
  planner: {
    ...
    lud16: "schlauskwab@minibits.cash",
  },
  mobility: {
    name: "mobility-lab",
  ...
  return features.map((feature, index) => {
    ...
    const baseProperties =
      feature.properties && typeof feature.properties === "object"
        ...
        : {};
    const description =
      typeof baseProperties.description === "string" &&
      baseProperties.description.trim().length > 0
...
function datasetAddress(pubkey: string, datasetId: string): string {
...
function withDatasetDescription(
  geoEventData: Awaited<ReturnType<typeof generateGeoEventData>>,
  description: string,
) {
  const featureCollection = parseFeatureCollection(
    geoEventData.content,
  ) as FeatureCollection & Record<string, unknown>;
  featureCollection.description = description;
  return {
    ...geoEventData,
...
function createRichDatasetDescription(options: {
  title: string;
  body: string[];
  image?: string;
  video?: string;
  ...
  const parts = [`## ${options.title}`, ...options.body];
  if (options.image) {
    parts.push("Image:");
    parts.push(`![${options.title}](${options.image})`);
  }
  if (options.video) {
    parts.push("Video:");
  ...
  if (options.reference) {
    parts.push(`Reference: [More context](${options.reference})`);
  }
  return parts.join("\n\n");
}
...
async function publishMapContext({
  ...
  contextCoordinates.forEach((coordinate) =>
    appendUniqueTag(tags, "c", coordinate),
  );
...
async function publishDataset({
  ...
  const datasetCoordinate = coordinate(
    GEO_EVENT_KIND,
    identity.pubkey,
    datasetId,
  );
  ...
  return {
    ...
    address: datasetAddress(identity.pubkey, datasetId),
    name: datasetName,
    ownerPubkey: identity.pubkey,
    featureCollection,
    bbox: parseBbox(tags),
...
async function publishProposal({
  ...
  console.log(
    `  Proposal published: "${description}" by ${identity.label} -> ${targetDataset.name}`,
  );
}
...
async function publishSeedCommentThread(
  ...
  for (const [threadIndex, threadSpec] of threadSpecs.entries()) {
    const author =
      identities[(seedIndex + threadIndex) % identities.length] ??
      identities[0];
    if (!author) continue;
    ...
    for (const [replyIndex, replySpec] of threadSpec.replies.entries()) {
      let replyAuthor =
        identities[
          (seedIndex + threadIndex + replyIndex + 1) % identities.length
        ] ?? identities[0];
      if (!replyAuthor) continue;
      if (replyAuthor.pubkey === author.pubkey && identities.length > 1) {
        replyAuthor =
          identities[
            (seedIndex + threadIndex + replyIndex + 2) % identities.length
          ] ?? replyAuthor;
      }
  ...
  console.log(
    `  Comments published: ${target.name} (${publishedCount} events)`,
  );
...
function mutateFeatureCollectionForProposal(
  ...
  switch (geometry.type) {
    ...
    case "GeometryCollection": {
      const pointLike = geometry.geometries.find(
  ...
  if (firstFeature.properties && typeof firstFeature.properties === "object") {
    (firstFeature.properties as Record<string, unknown>).seed_proposal =
      `proposal-${seedFactor}`;
  }
...
function createRailCorridorFeatureCollection(): FeatureCollection &
...
function createBikeNetworkFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  return {
    type: "FeatureCollection",
...
function createInvalidValidationFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  return {
    type: "FeatureCollection",
...
function createWaterQualityStationsFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  return {
    type: "FeatureCollection",
...
function createRiverSegmentsFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  return {
    type: "FeatureCollection",
...
function createHeritageHotspotsFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  return {
    type: "FeatureCollection",
...
function createAccessibleToiletsFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  const features: Feature[] = [
    {
      ...
      properties: {
        ...
        country: "DE",
        description:
          "High-frequency station toilet with step-free entrance and elevator access.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "US",
        description:
          "Central Manhattan facility frequently used for accessibility route planning.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "JP",
        description:
          "Modern universal-design toilet with broad turning radius.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "ZA",
        description:
          "Tourist-heavy, step-free facility with wide cubicle access.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "AU",
        description:
          "Transit-adjacent restroom with ramped approach and accessible cabin.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "BR",
        description:
          "Park toilet with tactile paving and wheelchair-friendly layout.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "CA",
        description:
          "Seasonal high-capacity accessible restroom near waterfront loop.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "AE",
        description:
          "Promenade-access facility for accessible waterfront itineraries.",
      },
...
function createHikingTrailsFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  const features: Feature[] = [
    {
      type: "Feature",
      ...
      properties: {
        ...
        country: "IT",
        description:
          "High alpine route with exposed ridgeline and panoramic views.",
      },
      geometry: {
        type: "LineString",
        coordinates: [
    ...
    },
    {
      ...
      properties: {
        ...
        country: "NP",
        description:
          "High-altitude route with thin-air conditions and ridge villages.",
      },
      geometry: {
        type: "LineString",
        coordinates: [
    ...
    },
    {
      ...
      properties: {
        ...
        country: "NZ",
        description:
          "Volcanic terrain with rapid weather shifts and long exposure.",
      },
      geometry: {
        type: "LineString",
        coordinates: [
    ...
    },
    {
      ...
      properties: {
        ...
        country: "MA",
        description:
          "Mountain forest circuit through cedar valleys and ridge paths.",
      },
      geometry: {
        type: "LineString",
        coordinates: [
...
function createSurfSpotsFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  const features: Feature[] = [
    {
      type: "Feature",
    ...
    },
    {
      ...
      properties: {
        ...
        country: "US",
        description:
          "World-class reef break requiring expert-level positioning.",
      },
    ...
    },
    {
      ...
      properties: {
        ...
        country: "FR",
        description:
          "Powerful hollow waves; benchmark spot for advanced surfers.",
      },
...
function createBestBeachesFeatureCollection(): FeatureCollection &
  Record<string, unknown> {
  const features: Feature[] = [
    {
      type: "Feature",
      ...
      properties: {
        ...
        country: "BR",
        description:
          "Urban mega-beach with extensive services and high visitation.",
      },
      geometry: {
        type: "Polygon",
        coordinates: [
    ...
    },
    {
      ...
      properties: {
        ...
        country: "MX",
        description:
          "Long Caribbean beachfront popular for recreation and tourism.",
      },
      geometry: {
        type: "Polygon",
        coordinates: [
    ...
    },
    {
      ...
      properties: {
        ...
        country: "US",
        description:
          "Accessible urban beach with broad amenities and gentle waves.",
      },
      geometry: {
        type: "Polygon",
        coordinates: [
...
async function seedData() {
  ...
  const planner = await createIdentity(
    "planner",
    devUser1.sk,
    USER_PROFILES.planner,
  );
  const mobility = await createIdentity(
    "mobility",
    devUser2.sk,
    USER_PROFILES.mobility,
  );
  const heritage = await createIdentity(
    "heritage",
    devUser3.sk,
    USER_PROFILES.heritage,
  );
  ...
  const STATE_DATASET_IDS = {
    "mecklenburg-vorpommern": "seed-state-mecklenburg-vorpommern",
    brandenburg: "seed-state-brandenburg",
    sachsen: "seed-state-sachsen",
    "sachsen-anhalt": "seed-state-sachsen-anhalt",
    thueringen: "seed-state-thueringen",
  } as const;
  const RAIL_DATASET_ID = "seed-rail-corridors-berlin-brandenburg";
  const BIKE_DATASET_ID = "seed-berlin-mobility-spines";
  const INVALID_RAIL_DATASET_ID = "seed-invalid-rail-context-polygons";
  const WATER_QUALITY_DATASET_ID = "seed-water-quality-stations";
  const RIVER_SEGMENTS_DATASET_ID = "seed-river-segments-reference";
  const HERITAGE_HOTSPOTS_DATASET_ID = "seed-heritage-hotspots";
  const ACCESSIBLE_TOILETS_DATASET_ID = "seed-accessible-public-toilets";
  const HIKING_TRAILS_DATASET_ID = "seed-global-hiking-trails";
  const SURF_SPOTS_DATASET_ID = "seed-global-surf-spots";
  const BEST_BEACHES_DATASET_ID = "seed-global-best-beaches-guide";
...
Foreign geometry attachments stay hidden.`,
  ...
  for (const state of STATE_SEEDS) {
    ...
    const published = await publishDataset({
      identity: planner,
      datasetId: STATE_DATASET_IDS[state.slug],
      geoEventData: withDatasetDescription(
        geoEventData,
        createRichDatasetDescription({
          title: `${state.name} baseline`,
          body: [
            `Administrative boundary dataset for **${state.name}** used in seeded inspect-mode demos.`,
            "These seeded boundaries anchor the authored context references and provide a canonical overlay for comparison.",
          ],
          image:
  ...
  const railDataset = await publishDataset({
    ...
    geoEventData: withDatasetDescription(
      ...
      createRichDatasetDescription({
        ...
        body: [
          "Primary corridor dataset for the required rail-quality context.",
          "It is pinned as a sticky reference and used as the canonical line-based validation example.",
        ],
        image:
  ...
  const bikeDataset = await publishDataset({
    ...
    geoEventData: withDatasetDescription(
      ...
      createRichDatasetDescription({
        title: "Mobility spine lines",
        body: [
          "Secondary line dataset contributed by another author to exercise open attachments in the rail-quality context.",
        ],
        image:
  ...
  const invalidRailContextDataset = await publishDataset({
    identity: heritage,
    datasetId: INVALID_RAIL_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        title: "Invalid rail-context demo",
        body: [
          "Intentionally invalid polygon overlays attached to a line-only context to demonstrate warnings and strict filtering.",
        ],
        image:
  ...
  const waterQualityDataset = await publishDataset({
    identity: mobility,
    datasetId: WATER_QUALITY_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        ...
        body: [
          "Seed monitoring stations for the optional river-health validation context.",
        ],
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a9/Example.jpg/640px-Example.jpg",
        video: "https://www.youtube.com/watch?v=aqz-KE-bpKQ",
  ...
  const riverSegmentsDataset = await publishDataset({
    identity: heritage,
    datasetId: RIVER_SEGMENTS_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        ...
        body: [
          "Supporting line reference set intentionally attached to the point-constrained river-health context.",
        ],
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Fronalpstock_big.jpg/640px-Fronalpstock_big.jpg",
        video: "https://www.youtube.com/watch?v=ScMzIvxBSi4",
  ...
  const heritageHotspotsDataset = await publishDataset({
    identity: heritage,
    datasetId: HERITAGE_HOTSPOTS_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        title: "Heritage hotspots",
        body: [
          "Curated heritage zones and landmarks used as the authoritative sticky lane in the heritage context.",
        ],
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Windows_Settings_app_icon.png/512px-Windows_Settings_app_icon.png",
        video: "https://www.youtube.com/watch?v=jNQXAC9IVRw",
  ...
  const accessibleToiletsDataset = await publishDataset({
    identity: planner,
    datasetId: ACCESSIBLE_TOILETS_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        ...
        body: [
          "Starter accessibility dataset pinned inside the required public-toilets context.",
        ],
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5f/Alberta_Banff_National_Park_Sunshine_Meadows.jpg/640px-Alberta_Banff_National_Park_Sunshine_Meadows.jpg",
        video: "https://www.youtube.com/watch?v=rUWxSEwctFU",
  ...
  const hikingTrailsDataset = await publishDataset({
    identity: heritage,
    datasetId: HIKING_TRAILS_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        title: "Hiking trails",
        body: [
          "Seed route network pinned in the hiking context and open for compatible community additions.",
        ],
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a9/Example.jpg/640px-Example.jpg",
        video: "https://www.youtube.com/watch?v=aqz-KE-bpKQ",
  ...
  const surfSpotsDataset = await publishDataset({
    identity: mobility,
    datasetId: SURF_SPOTS_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        title: "Surf spots",
        body: [
          "Starter surf-break catalog for the optional global surf context.",
        ],
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Fronalpstock_big.jpg/640px-Fronalpstock_big.jpg",
        video: "https://www.youtube.com/watch?v=ScMzIvxBSi4",
  ...
  const bestBeachesDataset = await publishDataset({
    identity: heritage,
    datasetId: BEST_BEACHES_DATASET_ID,
    geoEventData: withDatasetDescription(
      await generateGeoEventData(undefined, {
      ...
      createRichDatasetDescription({
        title: "Best beaches",
        body: [
          "Curated beach guide dataset used as the sticky authored lane for the closed beaches context.",
        ],
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Windows_Settings_app_icon.png/512px-Windows_Settings_app_icon.png",
        video: "https://www.youtube.com/watch?v=jNQXAC9IVRw",
  ...
  for (const [index, target] of commentTargets.entries()) {
    seededCommentCount += await publishSeedCommentThread(
      target,
      commentAuthors,
      index,
    );
  ...
  for (const [index, dataset] of allPublishedDatasets.entries()) {
    const proposalAuthor = pickProposalAuthor(
      dataset,
      proposalIdentities,
      index,
    );
  ...
  console.log(
    `[Seed] Published 3 users, ${totalContexts} contexts, ${totalDatasets} datasets, ${seededCommentCount} comment events, and ${proposalCount} proposals.`,
  );
  process.exit(0);
}
