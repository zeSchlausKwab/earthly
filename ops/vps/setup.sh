...
missing_commands="$(ssh "$remote" '
  ...
  missing=""
  for command_name in bun go pm2 curl tar sha256sum install caddy docker; do
...
if [[ -n "$missing_commands" ]]; then
  ...
  echo "Install them using docs/PRIVATE-MAPS-DEPLOYMENT.md, then rerun this command" >&2
  exit 1
fi
