    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          console.warn(
            `Overpass ${endpoint} returned ${response.status}, trying next...`,
          );
      ...
      console.log(
        `✅ Overpass ${endpoint} returned ${data.elements?.length ?? 0} elements`,
      );
      if (data.elements?.length > 0) {
        console.log(
          `   Sample element types: ${data.elements
            .slice(0, 5)
            .map((e: any) => e.type)
            .join(", ")}`,
        );
      ...
      lastError = err;
      console.warn(
        `Overpass ${endpoint} failed: ${err.message}, trying next...`,
      );
    }
  }
  ...
  return filterGroups
    .flatMap((filters) => {
      ...
      return elementTypes.map(
        (elementType) => `  ${elementType}${filterStr}${areaSelector};`,
      );
    })
    .join("\n");
}
...
function isPolygonWay(way: OverpassWay): boolean {
  ...
  return (
    areaTags.some((tag) => way.tags?.[tag] !== undefined) ||
    way.tags?.area === "yes"
  );
}
...
function relationBoundaryToGeometry(segmentsByRole: {
  outer: Position[][];
  inner: Position[][];
}): Geometry | null {
...
function pickBestBoundaryRelation(
  ...
  const scored = relations.map((relation) => {
    ...
    const contains = candidates.some((value) => value.includes(normalized))
      ? 1
      : 0;
...
export async function findAdministrativeBoundaryRelation(params: {
  countryCode?: string;
  name?: string;
  adminLevel?: number;
}): Promise<{
  relationId: number;
  relation: OverpassRelation;
  candidates: OverpassRelation[];
}> {
  const adminLevel = Math.max(
    2,
    Math.min(12, Math.floor(params.adminLevel ?? 2)),
  );
