...
- `AI_SUITE_HEADLESS=1` — run without a visible browser.
- `CI=1` — enables CI retries and headless execution.
...
through Earth's real browser-extension login control and can sign local test events.

Publishing and destructive tasks should eventually run in an isolated managed environment. Until
that adapter exists, do not add them to the default smoke suite.
