...
**Responsive (September 6 refinement).** The existing desktop toolbar stays on one row and measures the space left by its rendered controls inside the canvas, not just the window width. Draw and Edit progressively release individual shortcuts instead of waiting for an entire group to fit. Selection, Point, Line, Area and Undo/Redo have first priority; advanced drawing/editing shortcuts follow. The labeled Draw/Edit menus retain the full command catalog for keyboard navigation and commands not yet exposed inline. File, Audience/Publish and Thread stay reachable; compact location search expands only after editing shortcuts fit. Secondary tools use the labeled More menu. Opening or resizing a margin recalculates available space without resetting tool state. These measured priorities supersede the sketch's approximate 660/940px cutoffs. On a phone the toolbar is replaced by the edit dock (§13), whose More menu carries the same commands.
...
### Conversation context and direct Story publishing — 2026-09-11

- The account navigation button shows the signed-in avatar and name (short public key until a profile loads), or **Sign in** when signed out. It still opens the account popover without changing the current route.
- One inline disclosure, **AI can edit N · References N**, replaces the editing popover and bottom reference picker. Its two stacked sections are explicit permission boundaries: editable local Maps/Stories above, read-only sources and search below. Both lists remain scrollable within the bounded chat header; the disclosure opens during a drag.
- Browse rows, search results, draft rows, and editable geometry rows expose a dedicated grip. Dragging into a section adds that role; clicking the grip offers the same actions on touch and keyboard. These gestures never switch the visible editor, camera, conversation, or chat side. Search remains available without dragging. Message-only geometry and files live under **Attach to message**, separate from persistent references.
- Adding a foreign Map or Story for editing prepares a proposal, not permission to overwrite its author. A Map copy is an explicit separate action. Feature-only transfers retain their exact selector, including when moved back into references. Removing edit access does not delete the saved draft; discard and publish remain separate row actions. Changing an account mid-drag invalidates the transfer; changing accounts/conversations while an action loads requires retry. Running responses keep their captured permissions and sources fixed.
- Local Story references capture their narrative at Send just as local Map references capture geometry. Profile references are read-only snapshots. These sources are sent to the configured AI provider and never implicitly published. Removing a reference affects future messages, not prior messages or Story citations.
- A Story's publish icon publishes directly from the editable list, using its saved content and flushing any mounted editor first. Existing owned Stories publish their changes in one click; foreign Stories retain the explicit proposal flow. Publishing preserves chat, placement, route, and unsent composer text. Retained publication fingerprints distinguish **Published** from **Unpublished changes**, including edits after acknowledgement.
- A Story referencing unpublished local Maps asks once: **Publish Story and N maps**. Approval covers the captured dependencies and Story; it does not authorize other sources. Account/content checks guard asynchronous steps. A partial failure retains acknowledged Map publications and rewritten references so retry does not repeat completed work. Ordinary already-published references require no extra approval.

This is UI/local-draft work, not a protocol migration or partial rollout. The existing milestone-wide release gates remain applicable.
