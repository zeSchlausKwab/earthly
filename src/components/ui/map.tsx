...
function DefaultMarkerIcon() {
  return (
    <div className="relative h-4 w-4 rounded-full border-2 border-border bg-info shadow-lg" />
  );
}
...
export type LocateCoords = {
  longitude: number;
  latitude: number;
  /** GPS accuracy radius in meters (tracking mode only). */
  accuracy?: number;
};
...
type MapControlsProps = {
  ...
  className?: string;
  /**
   * If true, the locate button starts a continuous `watchPosition` session
   * (toggle to stop) and surfaces accuracy. Click while tracking → stop,
   * `onLocate(null)` fires so callers can clear the user-location marker.
   *
   * Default: false (one-shot `getCurrentPosition`, mapcn's stock behavior).
   */
  enableLocateTracking?: boolean;
  /**
   * Callback with user coordinates when located. In tracking mode this fires
   * on every position update; `null` is fired when the user stops tracking
...
function MapControls({
  ...
  // One-shot locate state (mapcn's original behavior).
  const [waitingForLocation, setWaitingForLocation] = useState(false);

  // Continuous tracking state — only used when enableLocateTracking is true.
  ...
  // Clear active watch on unmount or when tracking is disabled mid-session.
  useEffect(() => {
    return () => {
  ...
  // One-shot locate (mapcn's default behavior, used when !enableLocateTracking).
  const handleLocateOneShot = useCallback(() => {
    setWaitingForLocation(true);
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          const coords: LocateCoords = {
  ...
  // updates only emit via `onLocate` so the caller can move the marker.
  const stopLocateTracking = useCallback(() => {
    if (locateWatchRef.current !== null) {
      navigator.geolocation.clearWatch(locateWatchRef.current);
      locateWatchRef.current = null;
    }
  ...
  const startLocateTracking = useCallback(() => {
    ...
    locateWatchRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        setLocateStatus("tracking");
        if (isFirstFixRef.current && map) {
          map.flyTo({
            center: [coords.longitude, coords.latitude],
            zoom: 15,
            duration: 1000,
  ...
  }, [locateStatus, startLocateTracking, stopLocateTracking]);
  ...
  const locateButton = (() => {
    ...
    if (!enableLocateTracking) {
      return (
        <ControlButton
          onClick={handleLocateOneShot}
    ...
    const icon =
      locateStatus === "loading" ? (
        ...
        <LocateFixed className="size-4 text-info" />
      ) : locateStatus === "error" ? (
        <LocateOff className="size-4 text-destructive" />
      ) : (
        <Locate className="size-4" />
      );

    return (
      <ControlButton onClick={handleLocateTracking} label={trackingLabel}>
        {icon}
      </ControlButton>
    );
  })();
        ...
        <ControlGroup>
          ...
          >
            <Globe
              className={cn(
                "size-4",
                projectionMode === "globe" && "text-info",
              )}
            />
          </ControlButton>
        </ControlGroup>
  ...
  return (
    <ControlButton onClick={onClick} label="Reset bearing to north">
      ...
      >
        ...
        <path d="M12 2L8 12H12V2Z" className="fill-destructive/70" />
...
// LocateCoords is already exported via `export type LocateCoords = {...}` above.
