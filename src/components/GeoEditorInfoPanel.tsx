...
export function GeoEditorInfoPanelContent(props: GeoEditorInfoPanelProps) {
  ...
  const onAddFeatureCustomProperty = () => {
    ...
    editor.updateFeature(selectedFeature.id, {
      ...
      properties: {
        ...selectedFeature.properties,
        customProperties: { ...currentProps, [newFeatureProp.key]: newFeatureProp.value }
      }
    });
  ...
  if (viewMode === "view") {
    const headerTitle = viewCollection
      ? "Collection overview"
      : "Dataset overview";
            ...
            <section className="space-y-2">
              ...
              {viewCollectionEvents.length === 0 ? (
                <p className="text-xs text-gray-500">
                  No linked geo events are currently loaded. Listen for their
                  coordinates or load datasets first.
                </p>
              ) : (
        ...
        {viewDataset && !viewCollection && (
          <>
            <section className="rounded-lg border border-gray-200 p-3 space-y-2">
              ...
              <div className="text-[11px] text-gray-500">
                Owner: {viewDataset.pubkey.slice(0, 8)}…
                {viewDataset.pubkey.slice(-4)}
              </div>
      ...
      <section className="rounded-lg border border-gray-200 p-3 space-y-3">
        ...
        <div className="space-y-2">
          ...
          {blobReferences.map((reference) => {
            const isPreviewing =
              previewingBlobReferenceId === reference.id &&
                    ...
                    {reference.featureCount !== undefined && (
                      <div className="text-[11px] text-gray-500 space-x-2">
                        ...
                        {reference.geometryTypes &&
                          reference.geometryTypes.length > 0 && (
                            <span>
                              Geometry: {reference.geometryTypes.join(", ")}
                            </span>
                          )}
                      </div>
                    )}
        ...
        {selectionCount > 0 ? (
          <>
            ...
            <p>
              ...
              <strong>{multiSelectModifierLabel}</strong> while clicking or dragging with the Select tool to add to the selection.
            </p>
          </>
        ) : (
          <>
            ...
            <ul className="list-inside list-disc space-y-1">
              ...
              <li>Hold <strong>{multiSelectModifierLabel}</strong> to multi-select or drag to box-select.</li>
