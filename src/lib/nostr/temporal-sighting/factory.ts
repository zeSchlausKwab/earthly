/**
 * Factory for kind 37522 (Temporal Sighting Event).
 *
 ...
 * `expiration` setter is provided since sightings are expiry-bearing.
 */
...
import {
	DEFAULT_TEMPORAL_SIGHTING_CONTENT,
	isTemporalSighting,
	type TemporalSightingContent,
	type TemporalSightingEvent,
} from './helpers'

export class TemporalSightingFactory extends EntityFactory<typeof TEMPORAL_SIGHTING_KIND> {
	/** Start a new TemporalSighting from a content object. */
	static create(content: Partial<TemporalSightingContent> = {}): TemporalSightingFactory {
	...
	/** Continue editing an existing sighting (preserves d-tag). */
	static modify(event: TemporalSightingEvent): TemporalSightingFactory {
		if (!isTemporalSighting(event)) {
			throw new Error('TemporalSightingFactory.modify: event is not a kind 37522 sighting')
		}
	...
	sighting(content: Partial<TemporalSightingContent>): this {
		return this.chain((tpl) => {
			let parsed: Partial<TemporalSightingContent>
			try {
				parsed = JSON.parse(tpl.content) as Partial<TemporalSightingContent>
			} catch {
				parsed = {}
			}
			return {
	...
	/**
	 * Replace the NIP-92 imeta attachments (SPEC §6.1). Order is meaningful:
	 * the first attachment is the primary image shown in the map pin bubble.
	 */
