...
export class NDKGeoEvent extends NDKEvent {
	...
	set blobReferences(references: GeoBlobReference[] | undefined) {
		...
		references?.forEach((reference) => {
			...
			const scope = reference.scope === 'feature' ? `feature:${reference.featureId ?? ''}` : 'collection';
	...
	static async deleteDataset(ndk: NDK, dataset: NDKGeoEvent, reason?: string, signer?: NDKSigner): Promise<void> {
