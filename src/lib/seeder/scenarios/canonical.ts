/**
 * `seed canonical` — publishes real-world canonical datasets (sea cables,
 * meteorite landings, pipelines, nuclear power, airports, ports) from
 * `base-assets/base_rips/` to the target relay.
 *
 * Port of the retired scripts/seed_canonical_data.ts: same events (kind 37518
 * contexts + kind 37515 datasets, same d-tags/content/tags), but built through
 * the applesauce factories and published via the shared seeder relay layer —
 * no NDK compat shim, and the `--allow-remote` guard applies before this code
 * ever runs.
 *
 * Sub-seeders can be selected with `--only <name>`; a seeder whose source data
 * is missing (base_rips is gitignored) is skipped with a warning instead of
 * aborting the run.
 */
...
// Module-level run state, set by runCanonical(). Keeps the seeder bodies —
...
function withDTag(id: string) {
	return (tags: string[][]) => [['d', id], ...tags.filter((t) => t[0] !== 'd')]
}
...
async function seedSeaCables(signer: PrivateKeySigner, pubkey: string) {
...
async function seedMeteoriteLandings(signer: PrivateKeySigner, pubkey: string) {
...
function inferPipelineSource(raw: Record<string, any> | null): {
	...
	if (raw.osm_id || raw.man_made) return { source: 'OpenStreetMap', stroke: '#f97316' } // orange — OSM
	if (raw.nombre || raw.geo_point_2d) return { source: 'Brazil-ANP', stroke: '#a855f7' } // purple — Brazilian ANP
...
function normalizePipelineProps(raw: Record<string, any> | null): Record<string, unknown> {
	...
	// name
	const name =
		raw.name ?? raw.NAME ?? raw.PIPE_NAME ?? raw.nombre ?? raw.COMP_NAME ?? raw.Gasoducto ?? null
...
async function seedGasPipelines(signer: PrivateKeySigner, pubkey: string) {
	...
	for (const filename of allFiles) {
		...
		for (const feat of geo.features) {
			...
			const coords = (feat.geometry as LineString | MultiLineString).coordinates
...
function inferLiquidSource(raw: Record<string, any> | null): {
	...
	if (raw.CompanyCode || raw.BusinessUnitCL) return { source: 'US-NGL', color: '#3b82f6' } // blue — US/Canada NGL corporate GIS
	if ((raw.FID != null || raw.OWNER != null) && (raw.SUBSYS_NM || raw.SYS_NM))
		return { source: 'US-NGL', color: '#3b82f6' }
	if (raw.DUCTO || raw.EMPRESA || raw.TIPO_DUCTO) return { source: 'LatAm', color: '#ec4899' } // pink — South American
	if (raw.PIPE_NAME && (raw.REGION || raw.CONTENT || raw.LGTH_KM))
		return { source: 'SE-Asia', color: '#14b8a6' } // teal — SE Asia / Malaysia
	if (raw.Pipel_Name || raw.Approval_C || raw.Pipe_Seg) return { source: 'Shell', color: '#a855f7' } // purple — Shell/international upstream
	if (raw.ProjectID || raw.Product || raw.PrjType) return { source: 'GOIT', color: '#f97316' } // orange — Global Energy Monitor
...
function normalizeLiquidProps(raw: Record<string, any> | null): Record<string, unknown> {
	...
	// name
	const name =
		raw.name ?? raw.PIPE_NAME ?? raw.Pipel_Name ?? raw.DUCTO ?? raw.SYS_NM ?? raw.SUBSYS_NM ?? null
	if (name && typeof name === 'string' && name.trim()) p.name = name.trim()
	...
	// substance / product
	const sub =
		raw.substance ?? raw.CMDTY_DESC ?? raw.COMMODITY1 ?? raw.CONTENT ?? raw.TIPO_DUCTO ?? null
	if (sub && typeof sub === 'string' && sub.trim()) p.substance = sub.trim()
...
async function seedLiquidPipelines(signer: PrivateKeySigner, pubkey: string) {
...
async function seedNuclearPower(signer: PrivateKeySigner, pubkey: string) {
...
async function seedAirports(signer: PrivateKeySigner, pubkey: string) {
...
async function seedPorts(signer: PrivateKeySigner, pubkey: string) {
...
export const CANONICAL_SEEDERS = [
	'sea-cables',
	'meteorites',
	'gas-pipelines',
	'liquid-pipelines',
...
type SeederFn = (signer: PrivateKeySigner, pubkey: string) => Promise<void>

const SEEDER_FNS: Record<(typeof CANONICAL_SEEDERS)[number], SeederFn> = {
	'sea-cables': seedSeaCables,
	meteorites: seedMeteoriteLandings,
	'gas-pipelines': seedGasPipelines,
	'liquid-pipelines': seedLiquidPipelines,
	'nuclear-power': seedNuclearPower,
...
export async function runCanonical(ctx: SeederContext): Promise<void> {
	...
	if (only && !(CANONICAL_SEEDERS as readonly string[]).includes(only)) {
		throw new Error(`Unknown canonical seeder "${only}". Known: ${CANONICAL_SEEDERS.join(', ')}`)
	}
	...
	for (const name of CANONICAL_SEEDERS) {
		...
		try {
			await SEEDER_FNS[name](signer, pubkey)
		} catch (err) {
			if (isMissingDataError(err)) {
				console.warn(
					`  [skip] ${name} — source data not found under ${BASE_RIPS} (${(err as Error).message.slice(0, 100)})`,
				)
				continue
			}
			throw err
	...
	console.log(`\n[seed_canonical] All done: ${client.summary()}`)
}
