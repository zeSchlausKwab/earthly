...
export function createMapContextSystemMessage(): ChatMessage | null {
	...
	return {
		...
		content: [
			...
			'MAP LEGIBILITY — `label` is literal display text, never a template. Do not write `{name}`, `${name}`, or other placeholder expressions into it. Keep the real feature name in `name`; omit `label` on dense/bulk results and label only a small set of anchors that remain readable at the current zoom.',
