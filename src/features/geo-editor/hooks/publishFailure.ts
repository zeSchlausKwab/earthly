...
export function publishFailureMessage(action: string, error: unknown, retryAction = 'Publish'): string {
	const raw = error instanceof Error ? error.message : String(error)
	const detail = raw.replace(/\s+/gu, ' ').trim().slice(0, MAX_ERROR_DETAIL)
	return `Could not ${action}${detail ? `: ${detail}` : ''}. Tap ${retryAction} to retry; your draft is unchanged.`
}
