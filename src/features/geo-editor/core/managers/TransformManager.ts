...
import type { EditorFeature, TransformOptions, IManager } from '../types';
...
export class TransformManager implements IManager {
  ...
  scale(feature: EditorFeature, options: TransformOptions): EditorFeature {
    ...
    const scaled = turf.transformScale(feature, options.scale, { origin: options.center });
  ...
  translate(feature: EditorFeature, distance: number, direction: number): EditorFeature {
    const translated = turf.transformTranslate(feature, distance, direction, { units: 'meters' });
