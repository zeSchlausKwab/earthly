...
export class GeoEditor {
	...
	constructor(map: MapLibreMap, options: GeoEditorOptions = {}) {
		...
		this.snap = new SnapManager(this.options.snapDistance, this.options.snapToVertices, this.options.snapToEdges);
		...
		if (!this.map.getLayer(this.LAYER_LINE)) {
			this.map.addLayer({
				...
				paint: {
					...
					'line-dasharray': ['case', ['==', ['get', 'active'], true], ['literal', [2, 2]], ['literal', [1, 0]]]
				}
			});
		}
	...
	private onTouchEnd(e: MapTouchEvent): void {
		if (this.touchDrawInProgress && this.lastTouchPoint && this.panLockEnabled && this.isDrawMode(this.mode)) {
	...
	private updateTransformDrag(pointer: Position): void {
		...
		if (state.type === 'rotate') {
			...
			updatedFeatures = state.baseFeatures.map((feature) => this.transform.move(feature, state.startPointer, pointer));
		}
	...
	private onKeyDown(e: KeyboardEvent): void {
		...
		// Handle delete
		if (e.key === 'Delete' || (e.key === 'Backspace' && (this.mode === 'select' || this.mode === 'edit'))) {
			const selected = this.getSelectedFeatures();
	...
	combineSelectedFeatures(): boolean {
		...
		const parts = selected.flatMap((feature) => this.extractGeometryParts(feature.geometry, baseType));
	...
	splitSelectedFeatures(): boolean {
		const selected = this.getSelectedFeatures().filter((feature) => this.isMultiGeometry(feature.geometry.type));
	...
	deleteFeatures(featureIds: string[]): void {
		const deleted = featureIds.map((id) => this.features.get(id)).filter((f): f is EditorFeature => f !== undefined);
	...
	private extractGeometryParts(geometry: Geometry, base: 'Point' | 'LineString' | 'Polygon'): any[] {
		if (base === 'Point') {
	...
	private getAdjustedPositionFromScreenPoint(point: ScreenPoint, excludeFeatureIds: string[] = []): Position {
	...
	private getSelectionCentroid(features: EditorFeature[] = this.getSelectedFeatures()): Position | null {
	...
	getSelectedFeatures(): EditorFeature[] {
		...
		return selectedIds.map((id) => this.features.get(id)).filter((f): f is EditorFeature => f !== undefined);
	}
	...
	private shouldShowCursorIndicator(): boolean {
		return this.mode === 'draw_point' || this.mode === 'draw_linestring' || this.mode === 'draw_polygon';
	}
	...
	private updateCursorIndicator(position?: Position): void {
		...
		if (position && position[0] !== undefined && position[1] !== undefined && this.shouldShowCursorIndicator()) {
			source.setData({
	...
	private getFeatureCollection(): FeatureCollection {
		...
		const currentDrawFeature = this.drawLineMode.getCurrentFeature() || this.drawPolygonMode.getCurrentFeature();
	...
	destroy(): void {
		...
		try {
			if (this.map.getLayer(this.LAYER_SELECTION_BOX)) this.map.removeLayer(this.LAYER_SELECTION_BOX);
			...
			if (this.map.getLayer(this.LAYER_SELECTION_POINT)) this.map.removeLayer(this.LAYER_SELECTION_POINT);
			if (this.map.getLayer(this.LAYER_CURSOR)) this.map.removeLayer(this.LAYER_CURSOR);
			if (this.map.getLayer(this.LAYER_SELECTION_LINE)) this.map.removeLayer(this.LAYER_SELECTION_LINE);
			if (this.map.getLayer(this.LAYER_POINT)) this.map.removeLayer(this.LAYER_POINT);
			if (this.map.getLayer(this.LAYER_SELECTION_FILL)) this.map.removeLayer(this.LAYER_SELECTION_FILL);
