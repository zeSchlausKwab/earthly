...
import type { FeatureCollection, Feature, Position, Geometry } from "geojson";
...
export class GeoEditor {
    ...
    if (this.mode === "draw_point") {
      ...
      if (vertexFeatures.length > 0) {
        ...
        if (meta === "midpoint") {
          ...
          if (feature) {
            const updated = this.editMode.insertVertex(
              feature,
              path,
              clickPoint
            );
  ...
  private onMouseDown(e: MapMouseEvent): void {
    ...
    if (this.mode === "select") {
      ...
      if (
        this.isMultiSelectEvent(e.originalEvent) &&
    ...
    if (vertexFeatures.length > 0) {
      ...
      this.editMode.setDraggingVertex(featureId, path, [
        e.lngLat.lng,
        e.lngLat.lat,
      ]);
  ...
  private onMouseUp(_e: MapMouseEvent): void {
    ...
    if (
      this.isTouchLikeEvent(_e as any) &&
      this.isDrawMode(this.mode) &&
      !this.panLockEnabled
    ) {
      return;
  ...
  private onTouchEnd(e: MapTouchEvent): void {
    if (
      ...
      const position = this.getAdjustedPositionFromScreenPoint(
        this.lastTouchPoint
      );
  ...
  private updateSelectionDrag(e: MapMouseEvent): void {
    ...
    if (!this.selectionDragState.hasMoved) {
      const dx = Math.abs(
        this.selectionDragState.current.x - this.selectionDragState.start.x
      );
      const dy = Math.abs(
        this.selectionDragState.current.y - this.selectionDragState.start.y
      );
      if (dx > 3 || dy > 3) {
  ...
  private startTransformDrag(type: TransformDragType, pointer: Position): void {
    ...
    this.transformDragState = {
      ...
      startBearing:
        type === "rotate" ? turf.bearing(center, pointer) : undefined,
      baseFeatures,
      dragPanWasEnabled,
    };
    this.map.getCanvas().style.cursor = "grabbing";
  ...
  private updateTransformDrag(pointer: Position): void {
    ...
    if (state.type === "rotate") {
      const startBearing =
        state.startBearing ?? turf.bearing(state.center, state.startPointer);
      ...
      if (state.draggingVertex) {
        ...
        if (feature) {
          ...
          const snapResult = this.snap.snap(
            lngLat,
            this.getSnappableFeatures(),
            [featureId]
          );
          const newPosition = snapResult.snapped ? snapResult.point : lngLat;

          const updated = this.editMode.updateVertexPosition(
            feature,
            coordinatePath,
            newPosition
          );
  ...
  private onKeyDown(e: KeyboardEvent): void {
    ...
    if (
      e.key === "Delete" ||
      (e.key === "Backspace" &&
        (this.mode === "select" || this.mode === "edit"))
    ) {
      const selected = this.getSelectedFeatures();
  ...
  finishDrawing(): EditorFeature | null {
    ...
    if (this.mode === "draw_linestring") {
      feature =
        this.drawLineMode.onKeyDown({ key: "Enter" } as KeyboardEvent) ?? null;
    } else if (this.mode === "draw_polygon") {
      feature =
        this.drawPolygonMode.onKeyDown({ key: "Enter" } as KeyboardEvent) ??
        null;
    }
    if (feature) {
      this.addFeature(feature);
  ...
  canCombineSelection(): boolean {
    ...
    return selected.every(
      (feature) => this.getBaseGeometryType(feature.geometry.type) === baseType
    );
  }
  ...
  combineSelectedFeatures(): boolean {
    ...
    if (
      !selected.every(
        (feature) =>
          this.getBaseGeometryType(feature.geometry.type) === baseType
      )
    ) {
      return false;
    }
  ...
  private getSnappableFeatures(): EditorFeature[] {
    return this.getAllFeatures().filter(
      (feature) => feature.properties?.meta === "feature"
    );
  }
  ...
  private isTouchLikeEvent(
    ...
    if (typeof (original as any).pointerType === "string") {
      return (
        (original as any).pointerType === "touch" ||
        (original as any).pointerType === "pen"
      );
    }
  ...
  undo(): void {
    ...
    if (action.type === "create") {
      ...
      action.previousFeatures.forEach((f: EditorFeature) =>
  ...
  private updateCursorIndicator(position?: Position): void {
    const source = this.map.getSource(this.SOURCE_CURSOR) as
      | GeoJSONSource
      | undefined;
    if (!source) {
      return;
  ...
  private renderSelectionIndicator(): void {
    const source = this.map.getSource(this.SOURCE_SELECTION) as
      | GeoJSONSource
      | undefined;
    if (!source) {
      return;
  ...
  private renderGizmo(): void {
    const source = this.map.getSource(this.SOURCE_GIZMO) as
      | GeoJSONSource
      | undefined;
    if (!source) {
      return;
    ...
    const currentCenter =
      this.transformDragState?.center || this.getSelectionCentroid();
    if (!currentCenter) {
      source.setData({ type: "FeatureCollection", features: [] });
      return;
    }
  ...
  private renderSelectionBox(): void {
    const source = this.map.getSource(this.SOURCE_SELECTION_BOX) as
      | GeoJSONSource
      | undefined;
    if (!source) {
      return;
    ...
    const minX = Math.min(
      this.selectionDragState.start.x,
      this.selectionDragState.current.x
    );
    const minY = Math.min(
      this.selectionDragState.start.y,
      this.selectionDragState.current.y
    );
    const maxX = Math.max(
      this.selectionDragState.start.x,
      this.selectionDragState.current.x
