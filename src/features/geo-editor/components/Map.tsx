...
export const Map: React.FC<MapProps> = ({
  ...
  const [tileSourceMaxZoom, setTileSourceMaxZoom] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (!pmworldProtocolRegistered) {
      maplibregl.addProtocol("pmworld", async (params, abortController) => {
        ...
        const gh = lonLatToWorldGeohash(
          pmworldState.precision,
          center.lon,
          center.lat
        );
  ...
  useEffect(() => {
    ...
    const announcementUrl =
      mapSource.announcementUrl || "http://localhost:3333/api/announcement";
    ...
    (async () => {
      try {
        ...
        const announcedMaxZoom = Object.values(data).reduce(
  ...
  useEffect(() => {
    ...
    if (mapSource.type === "blossom" && tileSourceMaxZoom !== null) {
      ...
      if (url) {
        const pmtilesUrl = url.startsWith("pmtiles://")
          ? url
          : `pmtiles://${url}`;
        mapStyle = {
          version: 8,
