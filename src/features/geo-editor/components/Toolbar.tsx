...
export function Toolbar({
  ...
  const setSnappingEnabled = useEditorStore(
    (state) => state.setSnappingEnabled
  );
  ...
  const setShowMapSettings = useEditorStore(
    (state) => state.setShowMapSettings
  );
  ...
  const handleUndo = () => {
    editor?.undo();
    setHistoryState(
      editor?.history.canUndo() ?? false,
      editor?.history.canRedo() ?? false
    );
  };
  ...
  const publishButtons: ToolbarButton[] = [
    ...
    },
    {
      ...
      disabled:
        !datasetActions?.canPublishUpdate || datasetActions?.isPublishing,
        ...
        <div className="pointer-events-auto w-full max-w-md mx-auto">
          ...
          {mobileSearchOpen && (
            <div className="flex flex-col gap-2 rounded-lg bg-white/90 p-2 shadow-sm backdrop-blur">
              ...
              {searchError && (
                <div className="text-xs text-red-600 px-1">{searchError}</div>
              )}
            </div>
          )}
