...
interface EditorState {
  ...
  setMobileActiveState: (
    state: "datasets" | "info" | "tools" | "search" | "actions" | null
  ) => void;
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  setDatasetVisibility: (update) =>
    set((state) => ({
      datasetVisibility:
        typeof update === "function" ? update(state.datasetVisibility) : update,
    })),
  ...
  setBlobPreviewCollection: (blobPreviewCollection) =>
    set({ blobPreviewCollection }),
      ...
      set({
        ...
        blobDraftError:
          error instanceof Error
            ? error.message
            : "Failed to fetch external GeoJSON.",
      });
    }
  },
  ...
  previewBlobReference: async (id: string) => {
    ...
    try {
      const { payload, size, mimeType } = await fetchGeoJsonPayload(
        reference.url
      );
      const normalized = ensureFeatureCollection(payload);
      const collection = JSON.parse(
          ...
          ref.id === id
            ? {
                ...
                error:
                  error instanceof Error
                    ? error.message
                    : "Failed to load external GeoJSON.",
              }
            : ref
        ),
      }));
  ...
  removeBlobReference: (id: string) => {
    ...
    set((state) => {
      const newState: Partial<EditorState> = {
        blobReferences: state.blobReferences.filter(
  ...
  setViewCollectionEvents: (viewCollectionEvents) =>
    set({ viewCollectionEvents }),
  ...
  setShowTips: (showTips) =>
    set((state) => ({
      showTips:
        typeof showTips === "function" ? showTips(state.showTips) : showTips,
    })),
  setShowDatasetsPanel: (show) =>
    set((state) => ({
      showDatasetsPanel:
        typeof show === "function" ? show(state.showDatasetsPanel) : show,
    })),
  setShowInfoPanel: (show) =>
    set((state) => ({
      showInfoPanel:
        typeof show === "function" ? show(state.showInfoPanel) : show,
    })),
