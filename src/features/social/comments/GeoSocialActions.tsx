import { Heart, Loader2, MessageCircle, PencilLine, Share2, Zap } from 'lucide-react'
...
const ZapDialog = deferredSurface(async () => ({default:(await import('./ZapDialog.tsx')).ZapDialog}), 'payment options')
...
interface GeoSocialActionsProps {
	...
	compact?: boolean
	loadCounts?: boolean
	/** Reuse the existing count subscription when a containing list offers sorting. */
...
export function GeoSocialActions({
	...
	useEffect(() => {
		onReactionCountChange?.(reactionCount)
	}, [reactionCount, onReactionCountChange])
