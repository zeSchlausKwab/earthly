...
> **Update 2026-06-17 — tracked debt CLOSED (gap-closure pass):** All of the above were
> resolved in `03-REVIEW.md` (status: resolved). Security-relevant closures:
> **WR-02** eviction is now wired to chip-removal + chat switch/clear with an LRU cap
> (`MAX_INGEST_DATASETS=32`) — **T-03-07's lifetime sub-claim now fully holds**, not just the
> pre-parse cap. **WR-01** xlsx detached-buffer fallback now settles cleanly (no zero-byte
> re-parse), restoring T-03-05 fallback correctness. **WR-03** geocode throttle hoisted to a
> module-level clock shared across tools — the ~1 req/s T-03-15 policy now holds across calls.
> **WR-05** custom-provider vision probe no longer attaches the API-key bearer (key-leak avoidance).
> Full suite 244 pass / 0 fail. No register disposition changes — these strengthen existing CLOSED
> mitigations; `threats_open` remains 0.
