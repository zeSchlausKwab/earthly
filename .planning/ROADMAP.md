...
- [x] **Phase 13: Cross-Cutting** — Comment `K`/`k` root-scope widening + entity routing/addressing across all four new kinds (completed 2026-07-02)
...
**Wave 0**
  ...
  - [x] 11-01-PLAN.md — Nyquist RED baseline: geometry round-trip/bbox-g-derive/defensive-parse/c-emit/d-preserve, per-read-path expiry, observation-state classifier, rootKind=37522 comment emit
...
**Wave 2** *(blocked on Wave 1; 03 + 04 run in parallel — disjoint files)*
  ...
  - [x] 12-04-PLAN.md — Live-map render + browse rail: distinct beacon marker layer with data-driven live/stale/ended paint + dropExpired-before-source + freshest-per-{pubkey,d} id-tie-break (D-07/D-08), BeaconsPanel rail (Radio, own-beacon-pinned-top, D-12), AppSidebar wiring, seed fixtures (geometry/status, all 4 states + link-only) [BEACON-01/03/04] ✓ 2026-06-28 (4 files; build+biome green, suite 752/3 with the 3 fails exclusively Plan-05 fetchBeacon)
