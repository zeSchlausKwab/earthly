...
- [ ] **INGEST-07**: The app detects whether the selected model supports vision via layered detection (Ollama capabilities → modalities field → name heuristic → fail-safe to no-vision); the image-send affordance is disabled or marked uncertain when vision is unconfirmed, never silently sending images to a blind model.
