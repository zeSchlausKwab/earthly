...
use nostr::{Event, Filter, PublicKey, RelayUrl, Result, SubscriptionId, Timestamp};
    ...
    pub fn check_challenge(&mut self, event: &Event, relay_url: &RelayUrl) -> Result<(), String> {
        match event.tags.challenge() {
            Some(challenge) => {
                ...
                if !is_valid_auth_event(event, relay_url, challenge) {
                    return Err(String::from(
                        "authentication event has an invalid kind, relay, or challenge tag",
                    ));
                }
