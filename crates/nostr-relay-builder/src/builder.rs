...
/// Operation requested by an authenticated NIP-42 session.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Nip42PolicyAction {
    /// Read stored events through REQ, COUNT, or negentropy reconciliation.
    Read,
    /// Submit an event through EVENT.
    Write,
}

/// Authorizes the authenticated NIP-42 session public key for an operation.
///
/// Unlike [`WritePolicy`] and [`QueryPolicy`], this policy receives the key that signed the
/// connection's NIP-42 authentication event. This makes runtime pairing and revocation policies
/// possible without trusting the author or filter carried by a later client message.
pub trait Nip42Policy: fmt::Debug + Send + Sync {
    /// Check whether the authenticated session may perform `action`.
    fn admit_session<'a>(
        &'a self,
...
pub struct RelayBuilder {
    ...
    /// Authenticated NIP-42 session policy plugins
    pub(crate) nip42_plugins: Vec<Arc<dyn Nip42Policy>>,
    /// Tor hidden service
    #[cfg(feature = "tor")]
...
impl RelayBuilder {
    ...
    /// Add an authenticated NIP-42 session policy plugin.
    #[inline]
    pub fn nip42_policy<T>(mut self, policy: T) -> Self
    where
        T: Nip42Policy + 'static,
    {
        self.nip42_plugins.push(Arc::new(policy));
        self
    }
